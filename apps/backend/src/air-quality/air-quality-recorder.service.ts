...
export class AirQualityRecorderService implements OnModuleInit {
  ...
  async recordSnapshot(): Promise<{ inserted: number; skipped: number }> {
    const stations = await this.airKaz.getStations({ forceRefresh: true });
    if (stations.length === 0) {
      this.logger.warn("AirKaz returned 0 stations — snapshot skipped");
    ...
    this.logger.log(
      `AQ snapshot: ${inserted} inserted, ${skipped} skipped`,
    );
